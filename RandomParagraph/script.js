...
const getGenerate = () =>{
 ...
 if (isNaN(textSize.value) || textSize.value<0 || textSize.value>9) {
    const randomIndex = Math.floor(Math.random()*text.length)
    Paragraph.innerHTML = `<p class="para"> ${text[randomIndex]}</p>`
}else{
    const localtext = shuffle(text)
    const data = localtext.slice(0, textSize.value)
    console.log(data)

    const paras = data.map(
        (e)=>{
            return `<p class="para"> ${e}</p>`
        }
    )
    Paragraph.innerHTML = paras.join("");//join fu. use for convert array to string
}
}